...
try:
    ...
    messagebox.showerror("导入错误", "无法导入more_seller模块，请确保文件在同一目录下")
    sys.exit(1)
...
class WortenScraperGUI:
    ...
    def start_scraping(self):
        ...
        if not self.output_file_var.get().strip():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M")
            default_output = f"worten_seller_data_{timestamp}.xlsx"
    ...
    def run_scraper(self):
        ...
        try:
            ...
            # 启动爬虫，传递进度回调和停止检查回调
            more_seller.main(progress_callback=progress_callback, stop_check_callback=stop_check_callback)
