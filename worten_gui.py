...
try:
    ...
    messagebox.showerror("导入错误", "无法导入product_info模块，请确保文件在同一目录下")
    sys.exit(1)
...
class WortenScraperGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Worten爬虫控制台 v1.0")
    ...
    def start_scraping(self):
        ...
        if not self.output_file_var.get().strip():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M")
            default_output = f"worten_data_{timestamp}.xlsx"
    ...
    def run_scraper(self):
        ...
        try:
            ...
            # 启动爬虫，传递进度回调和停止检查回调
            product_info.main(progress_callback=progress_callback, stop_check_callback=stop_check_callback)
        ...
        try:
            while True:
                try:
                    ...
                    # 更新进度条和状态文本
                    if total > 0:
                        percentage = min((processed / total) * 100, 100)  # 确保不超过100%
                    ...
                    if processed > 0:
                        elapsed_time = time.time() - self.start_time if self.start_time else 0
                        if elapsed_time > 0:
                            self.status_bar.config(text=f"处理中... 已处理 {processed} 个任务，耗时 {elapsed_time:.1f} 秒")
